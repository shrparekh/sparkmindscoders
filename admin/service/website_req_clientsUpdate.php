...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $client_data = $_POST;
    $id = $client_data['id']; // Assuming you have an 'id' field in your form for the client record to update
    ...
    try {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['tmp_name']) {
            ...
            if (!empty($client_data['image'])) {
                deleteImage($client_data['image']);
            }

            // Convert and upload the new image to Cloudinary
            $client_data['image'] = convertAndUploadToCloudinary($_FILES['image']['tmp_name']);
        } else {
            $client_data['image'] = $client_data['Thumbnail_Image_available'];
        }
        ...
        if ($stmt->rowCount() > 0) {
            ...
            log_action($user_id, "Update Client Name " . $client_data['name'] . " In Database");
            echo json_encode(['success' => true, 'message' => '/admin/website/clients?success=Client updated successfully']);
        } else {
