...
$totalDataQuery = "SELECT COUNT(*) FROM portfolio";
$totalData = $conn->query($totalDataQuery)->fetchColumn();
$totalFiltered = $totalData;

// If there is a search parameter, modify the query
$sql = "SELECT portfolio.*, users.name AS user_name FROM portfolio INNER JOIN users ON portfolio.user_id = users.id WHERE 1=1";

if (!empty($request['search']['value'])) {
    $sql .= " AND (portfolio.name LIKE :search_value";
    $sql .= " OR portfolio.image LIKE :search_value";
    $sql .= " OR portfolio.image_alt LIKE :search_value";
    $sql .= " OR users.name LIKE :search_value";
    $sql .= " OR portfolio.created_at LIKE :search_value)";
}
...
if (!empty($request['order'])) {
    ...
    $sql .= " ORDER BY portfolio.created_at DESC";
}
