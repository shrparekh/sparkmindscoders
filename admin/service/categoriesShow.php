...
$totalDataQuery = "SELECT COUNT(*) FROM categories";
$totalData = $conn->query($totalDataQuery)->fetchColumn();
$totalFiltered = $totalData;

// If there is a search parameter, modify the query
$sql = "SELECT categories.*, users.name AS user_name FROM categories INNER JOIN users ON categories.user_id = users.id WHERE 1=1";

if (!empty($request['search']['value'])) {
    $sql .= " AND (categories.name LIKE :search_value";
    $sql .= " OR categories.slug LIKE :search_value";
    $sql .= " OR users.name LIKE :search_value";
    $sql .= " OR categories.created_at LIKE :search_value";
    $sql .= " OR categories.updated_at LIKE :search_value)";
}
...
while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
    ...
    $nestedData['slug'] = htmlspecialchars($row['slug']);
