...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_FILES['image']) || $_FILES['image']['error'] !== UPLOAD_ERR_OK) {
        ...
        if (empty($errors)) {
            ...
            if (isset($uploadResult['success']) && $uploadResult['success'] === false) {
                ...
                $portfolio_data['image'] = $uploadResult;
            }
        }
    }
    ...
    try {
        // Insert portfolio data into the 'portfolio' table
        $sql = "INSERT INTO portfolio (name, image, image_alt, published, user_id) 
        ...
        if ($stmt->rowCount() > 0) {
            ...
            log_action($user_id, "Add Portfolio Name " . $portfolio_data['name'] . " In Database");
            echo json_encode(['success' => true, 'message' => '/admin/website/portfolio?success=portfolio added successfully']);
        } else {
