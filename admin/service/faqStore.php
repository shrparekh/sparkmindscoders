...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $checkSlugSql = "SELECT COUNT(*) FROM faq WHERE name = :name";
        ...
        $insertSql = "INSERT INTO faq (name, faq_content, page_url, published, user_id) VALUES (:name, :faq_content, :page_url, :published, :user_id)";
        $insertStmt = $conn->prepare($insertSql);
        $insertStmt->bindParam(':name', $name, PDO::PARAM_STR);
        $insertStmt->bindParam(':faq_content', $faq_content, PDO::PARAM_STR);
        $insertStmt->bindParam(':page_url', $page_url, PDO::PARAM_STR);
        ...
        if ($insertStmt->rowCount() > 0) {
            ...
            log_action($user_id, "Add Faq Name " . $name . " In Database");
            // Success, respond with success message
            echo json_encode(['success' => true, 'message' => '/admin/website/faq?success=Successfully created the Faq']);
            exit;
        } else {
