...
$totalDataQuery = "SELECT COUNT(*) FROM faq";
$totalData = $conn->query($totalDataQuery)->fetchColumn();
$totalFiltered = $totalData;

// If there is a search parameter, modify the query
$sql = "SELECT faq.*, users.name AS user_name FROM faq INNER JOIN users ON faq.user_id = users.id WHERE 1=1";

if (!empty($request['search']['value'])) {
    $sql .= " AND (faq.name LIKE :search_value";
    $sql .= " OR faq.faq_content LIKE :search_value";
    $sql .= " OR faq.page_url LIKE :search_value";
    $sql .= " OR users.name LIKE :search_value";
    $sql .= " OR faq.created_at LIKE :search_value";
    $sql .= " OR faq.updated_at LIKE :search_value)";
}
...
if (!empty($request['order'])) {
    ...
    $sql .= " ORDER BY faq.created_at DESC";
}
...
while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
    ...
    $nestedData['faq_content'] = $row['faq_content'];
    $nestedData['page_url'] =$row['page_url'];
...
// id, name, faq_content, page_url, published, user_id, created_at, updated_at
?>
