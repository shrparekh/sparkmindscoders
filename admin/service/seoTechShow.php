...
$totalDataQuery = "SELECT COUNT(*) FROM codes";
$totalData = $conn->query($totalDataQuery)->fetchColumn();
$totalFiltered = $totalData;

// If there is a search parameter, modify the query
$sql = "SELECT codes.*, users.name AS user_name FROM codes INNER JOIN users ON codes.user_id = users.id WHERE 1=1";

if (!empty($request['search']['value'])) {
    $sql .= " AND (codes.name LIKE :search_value";
    $sql .= " OR codes.message LIKE :search_value";
    $sql .= " OR codes.type LIKE :search_value";
    $sql .= " OR users.name LIKE :search_value";
    $sql .= " OR codes.published LIKE :search_value";
    $sql .= " OR codes.created_at LIKE :search_value";
    $sql .= " OR codes.updated_at LIKE :search_value)";
}
...
while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
    ...
    $nestedData['message'] = htmlspecialchars($row['message']);
    $nestedData['type'] = ($row['type'] === 'meta') ? 'Header' : 'Footer';
