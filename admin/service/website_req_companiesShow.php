...
$totalDataQuery = "SELECT COUNT(*) FROM companies";
$totalData = $conn->query($totalDataQuery)->fetchColumn();
$totalFiltered = $totalData;

// If there is a search parameter, modify the query
$sql = "SELECT companies.*, users.name AS user_name FROM companies INNER JOIN users ON companies.user_id = users.id WHERE 1=1";

if (!empty($request['search']['value'])) {
    $sql .= " AND (companies.name LIKE :search_value";
    $sql .= " OR companies.page_url LIKE :search_value";
    $sql .= " OR users.name LIKE :search_value";
    $sql .= " OR companies.created_at LIKE :search_value";
    $sql .= " OR companies.updated_at LIKE :search_value)";
}
...
while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
    ...
    $nestedData['page_url'] = htmlspecialchars($row['page_url']);
