...
$totalDataQuery = "SELECT COUNT(*) FROM clients";
$totalData = $conn->query($totalDataQuery)->fetchColumn();
$totalFiltered = $totalData;

// If there is a search parameter, modify the query
$sql = "SELECT clients.*, users.name AS user_name FROM clients INNER JOIN users ON clients.user_id = users.id WHERE 1=1";

if (!empty($request['search']['value'])) {
    $sql .= " AND (clients.name LIKE :search_value";
    $sql .= " OR clients.image LIKE :search_value";
    $sql .= " OR clients.image_alt LIKE :search_value";
    $sql .= " OR users.name LIKE :search_value";
    $sql .= " OR clients.created_at LIKE :search_value)";
}
