...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $required_fields = ['name','comment', 'image_alt', 'published', 'user_id'];
    ...
    if (!isset($_FILES['image']) || $_FILES['image']['error'] !== UPLOAD_ERR_OK) {
        ...
        if (empty($errors)) {
            ...
            if (isset($uploadResult['success']) && $uploadResult['success'] === false) {
                ...
                $testimonial_data['image'] = $uploadResult;
            }
        }
    }
    ...
    try {
        // Insert testimonial data into the 'testimonial' table
        $sql = "INSERT INTO testimonial (name,comment, image, image_alt, published, user_id) 
                VALUES (:name,:comment, :image, :image_alt, :published, :user_id)";
        ...
        if ($stmt->rowCount() > 0) {
            ...
            log_action($user_id, "Add Testimonial Name " . $testimonial_data['name'] . " In Database");
            echo json_encode(['success' => true, 'message' => '/admin/website/testimonial?success=Testimonial added successfully']);
        } else {
